...
public class RemoveEventGUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RemoveEventGUI frame = new RemoveEventGUI();
	...
	public RemoveEventGUI() {
		...
		lblEventDate.setBounds(37, 31, 96, 13);
		...
		lblError.setBounds(107, 292, 530, 13);
		contentPane.add(lblError);
		
		btnDelete = new JButton(ResourceBundle.getBundle("Etiquetas").getString("RemoveEvent"));
		btnDelete.setBounds(212, 292, 225, 21);
		btnDelete.setEnabled(false);
		contentPane.add(btnDelete);
		
		lblError.setVisible(false); //Label only visible when error
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				BLFacade facade = MainGUI.getBusinessLogic();		
				facade.removeEvent(selectedEvent);
